...
export function validateProject(value: unknown): { ok: true; project: Project } | { ok: false; errors: string[] } {
  ...
  if (!isNonEmptyString(record.ownerEmail)) errors.push("ownerEmail must be a non-empty string");
  if (!isProjectStatus(record.status)) errors.push("status must be one of: planned, active, done");
  if (!isValidEstimateHours(record.estimatedHours)) errors.push("estimatedHours must be a non-negative number");
  ...
  const project: Project = {
    ...
    estimatedHours: record.estimatedHours as number,
